...
sunxi-nand-image-builder misc tool. Build/install either tool and keep it on
PATH, or set SNIB=/path/to/tool.
EOF
        exit 1
    fi
...
for cmd in sha256sum tar ssh ping ip dd mkimage sunxi-fel curl; do
    require_cmd "$cmd"
done
select_sunxi_nand_builder
sudo -n true
